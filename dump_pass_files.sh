...
do 
  ...
  do
    echo $pass | gpg --no-symkey-cache -d --batch --passphrase-fd 0 $file > $temp_dir/$(basename $file | sed 's/.gpg//')
    #echo $pass | gpg --cipher-algo AES256 --no-symkey-cache -d --batch --passphrase-fd 0 $file > $temp_dir/$(basename $file | sed 's/.gpg//')
    echo -e "\n----------------------\n"
  done
done
